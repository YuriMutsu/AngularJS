...
<body>
 ...
 if(isset($_POST['dangnhap'])){
        ...
        $tenkh      = $_POST["tendn"];
        $sotaikhoan  = $_POST["password"];
        
        //lay du lieu tu csdl ra để sánh
            $sql = "select * from khachhang where TENKH = '$tenkh' and SOTAIKHOAN = '$sotaikhoan' ";
			$query = mysqli_query($conn,$sql);
			$num_rows = mysqli_num_rows($query);
           
        if ($tenkh == "" || $sotaikhoan =="") {
            echo '<script> alert("Tên khách hàng hoặc password bạn không được để trống!!!!")</script>';
		}elseif ($num_rows ==0) {
                echo '<script>alert("Tên đăng nhập hoặc mật khẩu không đúng!!!!"); </script>';
            
			} else{
            
                   $sql1 = "select tenkh from khachhang where TENKH = '$tenkh' and SOTAIKHOAN = '$sotaikhoan'  ";
        ...
        //neu tim thay makh thi luu vao bang dang ky
            ...
            while($row = $result->fetch_assoc()) {
                ...
                $_SESSION['tenkh'] = $tenkh;
                 $_SESSION['QUYENSD'] = "khach hang";
                echo '<script>alert("Đăng Nhập tài khoản '.$tenkh.' thành công!!!!"); </script>';
    ...
    <div class="container">
     <div class="row panel panel-info">     
		...
		<div class =" panel-heading">
		<center><h4>ĐĂNG NHẬP TÀI KHOẢN NGƯỜI DÙNG</h4></center>
		</div>
		
		<div class="panel-body">
				<form class="sign-in col-sm-offset-1 col-sm-8" action="dangnhap.php" method="post">

			<div class="form-group ">
				<label class="control-label col-sm-4 col-sm-offset-1 " for="tendn" > Tên Khách hàng:</label>
				 <div class="col-sm-6">
                                <input type="text" class="form-control" name="tendn" placeholder="Nhâp vào tên đănh nhập">
                            </div>
			</div>	<br><br>			
		 <div class="form-group">
                            <label class="control-label col-sm-offset-1 col-sm-4" for="password">Mât khâu:</label>
                                <div class="col-sm-6 ">
                        ...
                        <div class="form-group">
                            <div class="col-sm-offset-5  col-sm-8">
                                ...
                                <a class="btn btn-primary" href="quenmatkhau.php">Quên Mật khẩu</a>   
                            </div>
                        </div>
