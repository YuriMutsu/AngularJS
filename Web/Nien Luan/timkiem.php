...
<body>
  ...
  <table border='1'class="col-sm-4 panel-body" >
      <tr>
<?php 
...
$d = 0;
        if(isset($_REQUEST['ok'])){
            ...
            if(empty($search)){
                echo '<script>alert("Nhập những gì bạn muốn tìm!")</script>';
        exit();
         }
            
                //TÌM KIẾM THEO TÊN SẢN PHẨM
                if(!is_int($search) || !is_float($search)){
                    //show du liệu
                        //tim kiem du lieu trong bang gan giong nhất
                            $sql = "SELECT * FROM sanpham WHERE TENSP like '%$search%'";
                            $result = mysqli_query($conn,$sql); //thuc thi cau truy vấn
                            $num = mysqli_num_rows($result); //đếm sô dòng trả về
            
                if($num > 0  &&  $search != ''){  
                    echo "<br>";
                        echo "Có $num kết quả được tìm thấy bởi từ khóa <b>$search</b>";
                        echo "<br>"; echo "<br>";
...
if($d%4 == 0)
   echo "</tr>";
        ...
        echo "Không tìm thấy kết quả cho từ khóa ".$search ;            
    } 
                }
